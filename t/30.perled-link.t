...
sub test_creolize {
    return Text::Creolize->new({type=>'perl'})->convert(@_)->result;
}
...
my $t = '';
$t .= '<p>You can use ';
$t .= $v->_build_anchor('[[internal links]]',$v->visit_link('internal links','internal links',$v));
$t .= '.</p>
<p>give the link a ';
$t .= $v->_build_anchor('[[internal links|different]]',$v->visit_link('internal links','different',$v));
$t .= ' name.</p>
';
return $t;
}
